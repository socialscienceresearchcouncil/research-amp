...
export default function edit( {
	...
	const { authorName, postDate } = useSelect(
		( select ) => {
			...
			const assocProfileIds = getEditedPostAttribute(
				'associated-profiles'
			);

			const assocProfileTerms =
				typeof assocProfileIds !== 'undefined' && assocProfileIds.length
					? getEntityRecords( 'taxonomy', 'ramp_assoc_profile', {
							include: assocProfileIds,
							context: 'view',
					  } )
					: [];
			...
			const authorNames =
				null !== assocProfileTerms && assocProfileTerms.length
					? assocProfileIds.map( ( termId ) => {
							const profileTerm = assocProfileTerms.find(
								( term ) => termId === term.id
							);
	...
	const publicationDate =
		postDate ?? __( 'Publication Date', 'research-amp' );

	const byline = showPublicationDate
		? sprintf(
				__( 'By %1$s on %2$s', 'research-amp' ),
				displayName,
				publicationDate
		  )
		: sprintf( __( 'By %1$s', 'research-amp' ), displayName );

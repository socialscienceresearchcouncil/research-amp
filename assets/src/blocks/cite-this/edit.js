...
export default function edit( {
	...
	const { headingText, helpText } = attributes;
		...
		( select ) => {
			...
			return {
				citation:
					_citation ??
					__(
						'Author, Sample. "Example of a citation." March 3, 2005. Your Site. https://example.com/sample-citation',
						'research-amp'
					),
			};
		},
	...
	const helpTextValue =
		helpText ??
		__(
			'Copy and paste the text below to cite this item.',
			'research-amp'
		);
		...
		<>
			<div { ...blockProps }>
				<>
					...
					<div className="cite-this-citation">{ citation }</div>
				</>
			</div>
		</>
	);
